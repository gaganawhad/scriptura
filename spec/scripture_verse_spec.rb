  ...
  describe ScriptureVerse do
    describe '#initialize' do
      it 'raises an error if either of the arugments cannot be converted to an integer' do
        -> { ScriptureVerse.new(:a1, 1, 4) }.should raise_error(ArgumentError)
        -> { ScriptureVerse.new(3, :asdf, 4) }.should raise_error(ArgumentError)
        -> { ScriptureVerse.new(3, 2, :four) }.should raise_error(ArgumentError)
      end

      it 'raises an error if the book number is not within 1-66' do
        -> { ScriptureVerse.new(67, 34, 12) }.should raise_error(RuntimeError)
        -> { ScriptureVerse.new(0, 34, 12) }.should raise_error(RuntimeError)
      end

      it 'raises an error when the chapter does not exist' do
        -> { ScriptureVerse.new(19, 151, 1) }.should raise_error(RuntimeError)
      end

      it 'raises an error when the verse does not exist' do
        -> { ScriptureVerse.new(19, 119, 500) }.should raise_error(RuntimeError)
      end

      it 'does not raise an error when the verse exists' do
        -> { ScriptureVerse.new(32, 2, 2) }.should_not raise_error
      end
    ...
    describe '#normalize' do
      context 'it converts a scripture verse to a normalized unique value over the range of scripture verses' do
        it 'returns 1,001,001 for scripture verse referring to Genesis 1:1 ' do
          ScriptureVerse.new(1, 1, 1).normalize.should == 1_001_001
        end

        it 'returns 66,022,021 for scripture verse referring to Rev 22:21 ' do
          ScriptureVerse.new(66, 22, 21).normalize.should == 66_022_021
        end

        it 'returns 19,119,176 for scripture verse referring to Psalm 119:176 ' do
          ScriptureVerse.new(19, 119, 176).normalize.should == 19_119_176
        end
      end
    end
